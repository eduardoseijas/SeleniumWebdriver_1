...
@given(u'Abrimos el navegador en chrome - V1')
...
@when(u'Cargamos los datos "{user}" "{clave}"')
def step_impl(context,user,clave):
    global driver, f
    f = Libreria_Funciones_Globales(context.driver)
    f.Text_Mixto_Validado("xpath","//input[@id='user-name']",user,1)
    f.Text_Mixto_Validado("xpath","//input[@id='password']", clave, 1)


@then(u'ingresamos a la pag chrome -v1')
def step_impl(context):
    global driver, f
    f = Libreria_Funciones_Globales(context.driver)
...
@then(u'Seleccionamos el archivo en chrome -v1')
def step_impl(context):
    global driver, f
    f = Libreria_Funciones_Globales(context.driver)
